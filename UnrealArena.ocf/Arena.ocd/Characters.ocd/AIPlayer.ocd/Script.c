  ...
  if(iTeam)
  {
    for(var i=0 ; i < GetPlayerCount() ; i++)
      if(GetPlayerTeam(GetPlayerByIndex(i)) == iTeam)
        if(FindObject(Find_ID(__AI), Find_Owner(GetPlayerByIndex(i))))
...
global func AddBot(int iTeam)
{
  ...
  // Clonk erstellen
  var bot = CreateObject(UA_Clonk, 0,0, found);
...
global func RemoveBot(int iTeam)
{
  ...
  if(bot)
  {
  	if(bot->Contained())
  	{
    	if(bot->Contained()->GetID() == RelaunchContainer || bot->Contained()->GetID() == RelaunchContainerEx)
...
public func InitializePlayer(int iPlr, int iX, int iY, object pBase, int iTeam, id idExtra)
{
  ...
  if(FindObject(Find_ID(GetID()), Find_Owner(iPlr))) return(RemoveObject());
  // Ich übernehme
  SetOwner(iPlr);
...
public func Activity()
{
  ...
  for(var i=0, pCrew ; pCrew = GetCrew(GetOwner(), i) ; i++)
  {
    if(pCrew->Contained())
    {
      ...
      if(pCrew->Contained()->GetID() == RelaunchContainerEx) pCrew->Contained()->ContainedLeft();
    }
    if(pCrew->Contained())
    {
...
private func GetGoal() { return(FindObject(Find_Category(C4D_Goal))); }
