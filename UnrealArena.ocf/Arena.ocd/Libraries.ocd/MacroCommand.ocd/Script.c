	...
	for (var i = 0, iEff, checked; i < GetEffectCount("MacroCommand", this); i++)
	{
		...
		for (var mac in aMacroCommandList) 
			if (mac == iEff)
				checked = true;
		if (!checked)
			RemoveEffect(nil, this, iEff, true);
		checked = false;
	}
	...
	for (var j = 0; j < iterationLimit; ++j)
	{
		...
		for (var i = 0; i < pathcount; ++i)
		{
			...
			while ((pNext->GetPathCount()) == 1)
			{
				...
				if (pNext == pEnd)
					break;
				// f�r den gedachten Pfad muss noch gecheckt werden, ob da nicht Jetpack ben�tigt wird
		...
		if (good)
		{
			chosenone = aBest[Random(good)];
		}
		else if (neutral)
		{
			chosenone = aBest[Random(neutral)];
		}
		else if (bad)
		{
			chosenone = aBest[Random(bad)];
		}
		else
		{
		...
		if (pNext == pEnd)
		{
			DebugLog
			(
				"Success: Complete path consists of %d waypoints.",
				"waypoints",
...
func MoveAlongPath(object pCurrentWp, object pNextWp, int iNextStep)
{
	...
	if (pCurrentWp->GetPathJetpack(path))
		...
		(
			this,
			"StartJetpack",
			pCurrentWp->GetPathJetpack(path),
			0,
			pCurrentWp->GetPathJetpackFlag(path)
		);
...
func StartJetpack(int iDir)
{
	...
	if (!jetpack)
		return;
	// Richtung z�nden
	if (iDir == Jetpack_Left)
	// Links
	{
		SetDir(DIR_Left);
		jetpack->ControlLeftDouble(this);
	}
	if (iDir == Jetpack_UpLeft)
	// Oben links
	{
		SetDir(DIR_Left);
		jetpack->ControlUpDouble(this);
	}
	if (iDir == Jetpack_UpRight)
	// Oben rechts
	{
		SetDir(DIR_Right);
		jetpack->ControlUpDouble(this);
	}
	if (iDir == Jetpack_Right)
	// Rechts
	{
		SetDir(DIR_Right);
...
{
	if (!(pCurrentWp->GetArriveCommandCount(path)))
		return;
	for (var i = pCurrentWp->GetArriveCommandCount(path) - 1; i > -1; i--)
		AddCommand
		(
			pCurrentWp->GetArriveCommand(path, i, 0, 0, this),
			...
			pCurrentWp->GetArriveCommand(path, i, 4, 0, this),
			0,
...
func SetAggroLevel(int iLevel, int iDist, int iX, int iY, string text)
{
	...
	if (!effect)
		effect = AddEffect("Aggro", this, 50, 10, this);
	effect.var0 = iLevel;
	// Parameter
	// iDist, Default = 500
	if (!iDist)
...
func FxAggroFire(object pTarget, proplist no)
{
	...
	if ((!GetCommand() && !GetMacroCommand()) || level != 1 || this->~IsAiming())
	{
		...
		// Weg frei und keine Nahkampfwaffe?
		{
			var angle = Angle(GetX(), GetY(), target->GetX(), target->GetY());
			if (((!Inside(angle, 70, 120) && !Inside(angle, 250, 290)) || Contents()->GetFMData(FM_Aim) > 0) && maxdist != 300 && ObjectDistance(target) < 300)
			{
				if (!this->~IsAiming())
		...
		if (this->~IsAiming() && !this->~CheckAmmo
			(
				Contents()->GetFMData(FM_AmmoID),
				Contents()->GetFMData(FM_AmmoLoad),
				Contents(),
				this
			))
			this->~StopAiming();
...
func CheckIdleWeapon()
{
	...
	if (Contents()->~IsWeapon())
		if (Contents()->IsReloading())
			return;
	// Inventar nach Waffe durchsuchen, die man Nachladen k�nnte
	for (var i = 0, mode = 1, obj; obj = Contents(i); mode++)
	{
		...
		if (this->~CheckAmmo
		(
			obj->GetFMData(FM_AmmoID, mode),
			obj->GetFMData(FM_AmmoLoad, mode) - obj->GetAmmo(obj->GetFMData(FM_AmmoID, mode))
		))
		{
			mode = obj->GetFireMode();
			break;
		}
		// N�chsten Feuermodus pr�fen
		if (mode == obj->GetFireMode())
			continue;
...
func CheckInventory()
{
	for (var i = 0, obj; obj = Contents(i); i++)
	{
		...
		if (obj->~IsHazardGear())
		{
			...
			if (this->~HasGear(obj->GetGearType()))
			{
				DropObject(obj);
				continue;
			}
