...
func InitializePlayer(int iPlr, int iX, int iY, object pBase, int iTeam, id idExtra)
{
	//Log("AI Initialized for %d",iPlr);
	SetCategory(C4D_StaticBack);
	if (FindObject(Find_ID(GetID()), Find_Owner(iPlr)))
		return RemoveObject();
}
...
func Activity()
{
	...
	for (var i = 0, pCrew; pCrew = GetCrew(GetOwner(), i); i++)
	{
		if (pCrew->Contained())
		{
			// Clonk steckt in RelaunchContainerEx?
			if (pCrew->Contained()->GetID() == RelaunchContainerEx)
				pCrew->Contained()->ContainedUp(); // von ContainedLeft() geändert - er soll wenn möglich seine Klasse neu wählen, um sich besser der Teamstrategie anzupassen
			// Container ist verschwunden, weil es RelaunchContainerEx war?
			if (pCrew->Contained())
				// Clonk steckt in RelaunchContainer?
				if (pCrew->Contained()->GetID() == RelaunchContainer)
					...
					if (FindObject(WPCH))
					{
						...
						for (i = 0; i < pseudoSet->GetItemCount(); i++)
							Call(rule, "BuyItem", pseudoSet->GetItem(i), pCrew);
						// Menü weg
						rule->Finish(0, pCrew);
					}
		}
...
*/
	...
	if (!snipe)
	{
		...
		if (wp)
			pCrew->SetMacroCommand(this, "MoveTo", wp, 0, 0, 0, Aggro_Shoot);
		DebugLog("@Going to a sniper point", "tactics");
	}
	else
	{
		...
		var target;
		if (!pCrew->GetAggroTarget())
			target = pCrew->~UTBotAIFindEnemy(pCrew, 1500);
		if (target)
		{
