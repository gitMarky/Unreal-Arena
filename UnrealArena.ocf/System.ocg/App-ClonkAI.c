...
//public func FindBestWeapon( object pTarget )
{
	...
	if (selection)
		ShiftContents(this, 0, GetID(selection));
...
func FindPath(object pStart, object pEnd, bool fJetpack)
{
	...
	for (var i = 0; i < GetLength(aNodes); i++)
		aDistance[i] = -1;
	aDistance[this->GetArrayItemPosition(pCurrent, aNodes)] = 0;
	...
	while (GetLength(aSet) && (!bEnd))
	{
		...
		for (pNode in aSet) 
		{
			var dist = aDistance[this->GetArrayItemPosition(pNode, aNodes)];
		...
		for (pNode in aSet) 
		{
			if (aDistance[this->GetArrayItemPosition(pNode, aNodes)] == iBest)
		...
		var pNext = 0;
		var pathcount;
		(pCurrent->WAYP->GetPathCount());
		var iCurrent = this->GetArrayItemPosition(pCurrent,aNodes);

		// alle Nachbarknoten des besten Knotens nach kürzeren Wegen absuchen
		for(var i=0; i<pathcount; ++i)
		{
			pNext = (pCurrent->WAYP->GetPathTarget(i));
			var iNext = this->GetArrayItemPosition(pNext,aNodes);
	...
	aPath=[pEnd];
	pCurrent = pEnd;
	while( pCurrent = aPrev[this->GetArrayItemPosition(pCurrent,aNodes)])
	{
